...
本地导入面向 PDF、OFD 和图片等发票或报销材料，不导入 `.eml` 邮件文件。Concur 浏览器方式使用用户登录的 Edge，不需要 Client ID 或密钥；必须先用包内独立样例验证企业账号、表单映射和逐份附件追加能力。官方 API 方式保留为高级选项，需要企业批准的 OAuth 应用。两种方式都只创建未提交草稿，不自动提交报销单。
