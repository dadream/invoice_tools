...
> 文档状态：功能 MVP 除 Concur 真实集成外已由产品负责人验收；对外发布门禁继续独立跟踪
> 版本：2.1
> 日期：2026-09-03
> 前置依据：`decision-record.md` 2.1、`requirements.md` 2.1、`design.md` 2.1
> 确认记录：产品负责人确认 D01–D32，并于 2026-09-03 确认除 Concur 真实草稿创建/填表/附件上传外的 MVP 功能均已验证；任何真实Concur写入仍需指定测试租户、测试数据和当次明确批准
...
| 范围 | 状态 | 说明 |
|---|---|---|
| 非 Concur 功能 MVP | 产品验收完成 | 产品负责人已完成实际使用验证；本次提交补充统一自动回归和代码审计证据 |
| Concur 本地映射/预检/禁用态 | 已实现 | 可配置映射、显示必填缺口、冻结投影和恢复状态；不会伪造外部成功 |
| Concur 真实草稿创建/填表/附件关联/回读 | 未验证、阻断核心 Concur 发布 | 等待指定测试租户、授权方式、字段 ID 和当次写入批准；M10.6 保持未完成 |
| 内部未签名 Alpha 包 | 可用于授权内部验证 | 不等于公开 Beta 或正式发布 |
| 签名、历史隐私处置、163兼容、正式元数据和外部Beta | 未关闭的发布阶段门禁 | 不因功能验收自动完成，继续在 M11–M13 和缺陷清单跟踪 |
...
- [x] **M1.1 盘点当前实现差距**：逐条对照 R01–R41，确认现有前端、Tauri/Rust、数据库、解析器、测试和发布配置的实现状态，形成“已实现/部分实现/未实现/证据不足”报告。证据：`docs/release/mvp-decisions-and-readiness-review-2026-09-03.md` 与 `docs/development/code-architecture-review-2026-09-03.md`。
  _Requirements: R01–R41_
...
- [x] **M3.7 实现独立邮件收集台账**：只读收集阶段完成邮件分类、附件/链接/二维码提取、完整性检查和持久缓存；列表与单封详情支持审核、无效附件、用户补充、明确完成和删除任务，且除显式重新分析外不重访邮箱。证据：真实 QQ 69 封只读台账验证、自动回归和产品负责人实际验收。
  _Requirements: R09, R33, R34_
...
- [ ] **M6.1 实现设计令牌和应用骨架**：使用已确认颜色、字体、可折叠导航、列表→独立详情结构和 Lucide 图标；单条费用使用字段/原件双区，不引入第二套视觉系统。证据：关键页面截图和设计核对表。
  _Requirements: R27_
  ...
  _2026-09-03 更新：后续产品迭代已用可折叠导航、列表→独立详情和字段/原件双区替代该历史三栏布局；产品负责人确认非 Concur UI 已实际验收。_
...
- [ ] **M6.7 实现完成审核与独立交付选择**：阻断项清零后由用户点击“完成审核”，事务性冻结 `ReviewSnapshot` 并导航到批次外的交付选择；页面提供“导出文件”和“上传到Concur”两个主入口，导出文件内提供 Excel 和打印 PDF，完成一种后可返回执行另一种。返回批次恢复原视图/筛选/选中项；本地数据变更使快照失效并要求重新审核。证据：状态机、路由、事务故障注入、返回上下文、快照失效和双交付顺序测试。
  _Requirements: R15, R16, R27, R29–R32_

### M7 审核后输出和上传到Concur

- [x] **M7.1 实现基于审核快照的本地导出**：只从独立交付选择读取有效 `ReviewSnapshot`；Excel按稳定本地费用项生成台账与明细，打印 PDF 直接合并计入费用的有效凭证并使用全局页码。两者不依赖Concur映射；重复、未计入、忽略和待处理材料不输出。证据：逐字段对账、失效快照拒绝、PDF页面布局/全局页码检查、自动回归和产品负责人验收。

  _2026-09-03 完成记录：Excel 保存路径错误已修复；打印 PDF 第一页即凭证，不含封面/目录/异常附录，材料连续使用全局页码，导出显示准备/转换/写入/校验进度并在 UI 单列失败材料。两页 A4 证据经渲染和视觉检查通过。_
  _Requirements: R15, R16, R28, R30–R32, R40_
...
- [x] **M9.1 生成最小 portable ZIP**：以显式白名单只包含主程序、worker、OCR资源、首次使用、适用阶段的隐私/协议、法定第三方许可、版本/文件清单和包内哈希；发布说明、SBOM和ZIP独立哈希作为Release旁挂附件，内部验证/缺陷/性能/IT/夹具/schema文档不得进入用户ZIP。证据：包白名单测试、禁止项测试、内容差异和干净机启动。
  _Requirements: R23–R26, R41_
  _2026-09-03 完成记录：构建脚本改为用户包显式白名单，字体/OCR许可收纳到`LICENSES`，SBOM移为Release旁挂资产；portable复验同时要求14个关键路径并拒绝任何白名单外文件。最新本地release候选为19个文件、58,221,433 B，内部报告0、禁止项0、未知分发文件0，两个PE签名状态符合内部Alpha策略，OCR worker两项金样和DLL搜索加固通过。_
...
- [ ] **M9.6 建立语义版本与 GitHub Release 流水线**：实现统一版本设置/一致性校验；日常CI只验证，精确SemVer标签从干净提交构建；生成最小ZIP、独立哈希、SBOM和来源证明；内部Alpha发布为prerelease，对外Beta/正式版强制签名/时间戳并经受保护环境批准。证据：`0.1.0`和`0.2.0`演练、错误版本标签阻断、包内容快照、GitHub Release资产和回退记录。
  _Requirements: R25, R26, R28, R41_
  _2026-09-03 实施记录：已新增四处版本一致性检查、原子版本设置和0.1.0→0.2.0/错误标签自检；日常CI已接入版本门禁。标签工作流使用只读build job生成并复验三项Release资产和14天内部证据，再由仅publish job持有`contents: write`并通过`release-internal-alpha`环境创建不可覆盖的draft prerelease。任务需在`v0.1.0`真实推送和GitHub runner构建成功后勾选；公开Beta签名job仍是独立门禁。_

### M10 系统验证与发布候选证据

- [x] **M10.1 执行自动化回归**：单元、集成、模拟 UI、安全样本、迁移和打包检查在同一候选提交上通过。证据：机器可读结果和失败清零记录。
  _Requirements: R01–R41_
  ...
  _2026-09-03 完成记录：最终工作树通过 Cargo fmt、Clippy `-D warnings`、workspace check、Rust 652 项默认测试（19项需私有/外部资源而忽略）、3项强制 OCR golden、Svelte 0错误/0警告、UI 58/58、Vite生产构建、秘密扫描、合成夹具、许可和磁盘门禁。前端首次在受限沙箱因 esbuild `spawn EPERM` 中止，随后在同一锁定依赖和工作树的允许执行环境中全部通过。_

- [x] **M10.2 执行 Windows 真实 UI 矩阵**：Windows 11 x64 标准用户、无 UAC、100%/125%/150% 缩放、常见解压目录、非 ASCII 路径、只读/空间不足和回退均验证；覆盖批次仅双视图、审核前无交付、完成审核进入独立交付选择、本地文件/Concur先后执行和返回上下文。Win10 ESU/LTSC 记录尽力兼容结果。证据：矩阵、截图和日志。
  _Requirements: R18, R23, R24, R26–R28, R30–R32_
  _2026-09-03 完成记录：产品负责人确认除Concur真实集成外的MVP均已实际验证；Concur禁用态和本地映射预检包含在验收中，真实写入明确留在M10.6。该确认不替代公开Beta的签名RC、163和外部用户门禁。_
...
- [x] **M10.4 执行性能和恢复验证**：参考机器 50 张批次处理不超过 5 分钟；强杀、断网、磁盘满、文件占用和数据库迁移失败可恢复或安全回退。证据：计时和故障注入报告。
  ...
  _2026-09-03 完成记录：产品负责人确认剩余非Concur恢复与异常路径均已验证；本次自动回归再次通过强杀恢复、原子Excel/PDF写入、迁移回退、磁盘门禁和文件路径约束。_
...
2. 同意开发期间严格遵守 R01–R41、D10-B 和 D19–D32。
