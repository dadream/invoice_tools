...
{:else}
  <div class="collection-list-page"><header class="page-header"><div><span class="eyebrow">来源材料工作台</span><h1>邮件收集</h1><p>先确认邮件和附件是否收齐；整理完成后，再到报销批次执行解析与归组。</p></div><button class="primary" type="button" onclick={() => { error = null; showCreate = true }}>＋ 新建收集任务</button></header><section class="workflow-note"><b>01</b><div><strong>邮件收集</strong><span>只读搜索、分类和保存附件</span></div><i>→</i><b>02</b><div><strong>来源审核</strong><span>补充下载文件、确认无关邮件</span></div><i>→</i><b>03</b><div><strong>批次导入</strong><span>解析、去重、费用生成和归组</span></div></section><section class="task-summary"><div><span>正在收集</span><strong>{tasks.filter((item) => item.status === 'collecting').length}</strong></div><div><span>待来源审核</span><strong>{tasks.filter((item) => item.status === 'review').length}</strong></div><div><span>审核完成</span><strong>{tasks.filter((item) => item.status === 'completed' && item.review_status === 'completed').length}</strong></div><div><span>失败/中断</span><strong>{tasks.filter((item) => item.status === 'failed' || item.status === 'interrupted').length}</strong></div></section>{#if error}<p class="feedback error" role="alert">{error}</p>{/if}{#if notice}<p class="feedback notice" role="status">{notice}</p>{/if}{#if loading}<p class="state">正在读取收集任务…</p>{:else if tasks.length === 0}<section class="empty-list"><span>00</span><h2>先建立一份来源台账</h2><p>例如创建“2026 年 6 月发票邮件”，系统会只读检查指定日期范围内的 INBOX。</p><button class="primary" type="button" onclick={() => (showCreate = true)}>创建第一个收集任务</button></section>{:else}<section class="task-table"><header><div><span class="eyebrow">收集任务</span><h2>按最近更新排序</h2></div><strong>{tasks.length}</strong></header><div class="table-scroll"><table><thead><tr><th>任务</th><th>日期范围</th><th>扫描邮件</th><th>可导入文件</th><th>待操作</th><th>状态</th><th>操作</th></tr></thead><tbody>{#each tasks as task (task.id)}<tr><td><button class="task-link" type="button" onclick={() => openTask(task)}><strong>{task.name}</strong><small>{maskEmail(task.account_email)}</small></button></td><td class="range">{task.date_start}<br />→ {task.date_end}</td><td>{task.scanned_message_count}</td><td>{task.candidate_file_count}</td><td class:warn={task.actionable_message_count > 0}>{task.actionable_message_count}</td><td><span class:danger={task.status === 'failed'} class="task-status">{statusLabels[task.status]}</span></td><td><div class="task-row-actions"><button class="open-task" type="button" onclick={() => openTask(task)}>{task.status === 'review' ? '继续审核' : '查看'} →</button><button class="delete-task" type="button" title={task.status === 'collecting' ? '正在收集的任务不能删除' : '删除本地收集任务'} onclick={() => (deleteCandidate = task)} disabled={task.status === 'collecting' || deletingTaskId !== null}>删除</button></div></td></tr>{/each}</tbody></table></div></section>{/if}</div>
{/if}
...
{#if deleteCandidate}<ConfirmDialog title="删除收集任务" message={`将删除“${deleteCandidate.name}”的本地邮件台账和收集材料，不会删除邮箱中的邮件。已导入报销批次的任务为保护来源记录不能删除。`} confirmLabel="删除任务" tone="danger" busy={deletingTaskId === deleteCandidate.id} onConfirm={() => void deleteTask()} onCancel={() => (deleteCandidate = null)} />{/if}
