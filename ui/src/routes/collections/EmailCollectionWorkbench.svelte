<script lang="ts">
  ...
  type TaskSortKey = 'name' | 'date_start'
  type TaskSummaryFilter = 'collecting' | 'review' | 'completed' | 'exception'
  ...
  let deleteCandidate = $state<EmailCollectionTask | null>(null)
  ...
  let taskSortKey = $state<TaskSortKey | null>(null)
  let taskSortDirection = $state<SortDirection>('asc')
  let taskSummaryFilter = $state<TaskSummaryFilter | null>(null)
  ...
  const collectingTaskCount = $derived(tasks.filter((item) => item.status === 'collecting').length)
  const reviewTaskCount = $derived(tasks.filter((item) => item.status === 'review').length)
  const completedTaskCount = $derived(tasks.filter((item) => item.status === 'completed' && item.review_status === 'completed').length)
  const exceptionTaskCount = $derived(tasks.filter((item) => item.status === 'failed' || item.status === 'interrupted').length)
  const visibleTasks = $derived.by(() => {
    const query = taskSearchQuery.trim().toLocaleLowerCase()
    const filtered = tasks.filter((task) => {
      let matchesStatus = true
      if (taskSummaryFilter === 'exception') matchesStatus = task.status === 'failed' || task.status === 'interrupted'
      else if (taskSummaryFilter === 'completed') matchesStatus = task.status === 'completed' && task.review_status === 'completed'
      else if (taskSummaryFilter !== null) matchesStatus = task.status === taskSummaryFilter
      return matchesStatus && (!query || task.name.toLocaleLowerCase().includes(query))
    })
    if (taskSortKey === null) return filtered
    return [...filtered].sort((left, right) => {
      const compared = taskSortKey === 'name'
        ? left.name.localeCompare(right.name, 'zh-CN', { numeric: true, sensitivity: 'base' })
        : left.date_start.localeCompare(right.date_start)
      return taskSortDirection === 'asc' ? compared : -compared
    })
  })
  ...
  function toggleTaskSort(key: TaskSortKey) {
    if (taskSortKey === key) taskSortDirection = taskSortDirection === 'asc' ? 'desc' : 'asc'
    else { taskSortKey = key; taskSortDirection = 'asc' }
  }
  function taskSortIndicator(key: TaskSortKey): string {
    if (taskSortKey !== key) return '↕'
    return taskSortDirection === 'asc' ? '↑' : '↓'
  }
  function toggleTaskSummaryFilter(filter: TaskSummaryFilter) {
    taskSummaryFilter = taskSummaryFilter === filter ? null : filter
  }
  function changeTaskSummaryFilter(value: string) {
    taskSummaryFilter = value === 'all' ? null : value as TaskSummaryFilter
  }
  ...
  async function deleteTask() {
    if (!deleteCandidate || deletingTaskId !== null) return
    const task = deleteCandidate
    deletingTaskId = task.id
    error = null
    const result = await invokeSafe<void>('delete_email_collection_task', { taskId: task.id })
    deletingTaskId = null
    if (!result.ok) { deleteCandidate = null; error = describeError(result.error); return }
    deleteCandidate = null
    if (selectedTask?.id === task.id) selectedTask = null
    notice = `已删除本地收集任务“${task.name}”及其材料；邮箱中的邮件未被修改。`
    await loadTasks()
  }
...
{:else}
  <div class="collection-list-page">
    <header class="page-header">
      <div><span class="eyebrow">来源材料工作台</span><h1>邮件收集</h1><p>只读搜索邮件，分类和保存附件；审核并补充下载文件、确定无关邮件。</p></div>
      <button class="primary" type="button" onclick={() => { error = null; showCreate = true }}>＋ 新建收集任务</button>
    </header>
    <section class="task-summary" aria-label="收集任务状态筛选">
      <button class:active={taskSummaryFilter === 'collecting'} type="button" aria-pressed={taskSummaryFilter === 'collecting'} disabled={collectingTaskCount === 0} onclick={() => toggleTaskSummaryFilter('collecting')}><span>正在收集</span><strong>{collectingTaskCount}</strong></button>
      <button class:active={taskSummaryFilter === 'review'} type="button" aria-pressed={taskSummaryFilter === 'review'} disabled={reviewTaskCount === 0} onclick={() => toggleTaskSummaryFilter('review')}><span>待来源审核</span><strong>{reviewTaskCount}</strong></button>
      <button class:active={taskSummaryFilter === 'completed'} type="button" aria-pressed={taskSummaryFilter === 'completed'} disabled={completedTaskCount === 0} onclick={() => toggleTaskSummaryFilter('completed')}><span>审核完成</span><strong>{completedTaskCount}</strong></button>
      <button class:active={taskSummaryFilter === 'exception'} type="button" aria-pressed={taskSummaryFilter === 'exception'} disabled={exceptionTaskCount === 0} onclick={() => toggleTaskSummaryFilter('exception')}><span>失败/中断</span><strong>{exceptionTaskCount}</strong></button>
    </section>
    {#if error}<p class="feedback error" role="alert">{error}</p>{/if}
    {#if notice}<p class="feedback notice" role="status">{notice}</p>{/if}
    {#if loading}
      <p class="state">正在读取收集任务…</p>
    {:else if tasks.length === 0}
      <section class="empty-list"><span>00</span><h2>先建立一份来源台账</h2><p>例如创建“2026 年 6 月发票邮件”，系统会只读检查指定日期范围内的 INBOX。</p><button class="primary" type="button" onclick={() => (showCreate = true)}>创建第一个收集任务</button></section>
    {:else}
      <section class="task-table" aria-label="收集任务列表">
        <header><div class="task-list-tools"><label><span class="sr-only">搜索任务名称</span><input bind:value={taskSearchQuery} placeholder="搜索任务名称" /></label><select value={taskSummaryFilter ?? 'all'} aria-label="任务状态" onchange={(event) => changeTaskSummaryFilter(event.currentTarget.value)}><option value="all">全部任务</option><option value="collecting">正在收集</option><option value="review">待来源审核</option><option value="completed">审核完成</option><option value="exception">失败/中断</option></select></div><strong>共 {visibleTasks.length} 项</strong></header>
        <div class="table-scroll">
          <table>
            <thead><tr>
              <th aria-sort={taskSortKey === 'name' ? (taskSortDirection === 'asc' ? 'ascending' : 'descending') : 'none'}><button class="sort-header" type="button" onclick={() => toggleTaskSort('name')}>任务 <span aria-hidden="true">{taskSortIndicator('name')}</span></button></th>
              <th aria-sort={taskSortKey === 'date_start' ? (taskSortDirection === 'asc' ? 'ascending' : 'descending') : 'none'}><button class="sort-header" type="button" onclick={() => toggleTaskSort('date_start')}>日期范围 <span aria-hidden="true">{taskSortIndicator('date_start')}</span></button></th>
              <th>扫描邮件</th><th>可导入文件</th><th>待操作</th><th>状态</th><th>操作</th>
            </tr></thead>
            <tbody>
              {#each visibleTasks as task (task.id)}
                <tr><td><button class="task-link" type="button" onclick={() => openTask(task)}><strong>{task.name}</strong><small>{maskEmail(task.account_email)}</small></button></td><td class="range">{task.date_start}<br />→ {task.date_end}</td><td>{task.scanned_message_count}</td><td>{task.candidate_file_count}</td><td class:warn={task.actionable_message_count > 0}>{task.actionable_message_count}</td><td><span class:danger={task.status === 'failed'} class="task-status">{statusLabels[task.status]}</span></td><td><div class="task-row-actions"><button class="open-task" type="button" onclick={() => openTask(task)}>{task.status === 'review' ? '继续审核' : '查看'} →</button>{#if task.status !== 'completed'}<button class="delete-task" type="button" title={task.status === 'collecting' ? '正在收集的任务不能删除' : '删除本地收集任务'} onclick={() => (deleteCandidate = task)} disabled={task.status === 'collecting' || deletingTaskId !== null}>删除</button>{/if}</div></td></tr>
              {:else}<tr><td class="no-rows" colspan="7">当前筛选条件下没有收集任务。</td></tr>{/each}
            </tbody>
          </table>
        </div>
      </section>
...
{#if showCreate}<div class="modal-backdrop" role="presentation" onclick={(event) => event.target === event.currentTarget && (showCreate = false)}><div class="create-modal" role="dialog" aria-modal="true" aria-labelledby="create-collection-title"><header><h2 id="create-collection-title">新建收集任务</h2><button type="button" aria-label="关闭" onclick={() => (showCreate = false)}>×</button></header><form onsubmit={(event) => { event.preventDefault(); void createTask() }}><label><span>任务名称 *</span><input bind:value={taskName} maxlength="100" placeholder="例：2026 年 6 月发票邮件" /></label><label><span>当前邮箱</span><input value={session.email ?? '尚未配置会话授权码'} disabled /></label><div class="date-fields"><label><span>开始日期 *</span><input type="date" bind:value={dateStart} /></label><label><span>结束日期（不含）*</span><input type="date" bind:value={dateEnd} /></label></div><aside><strong>收集阶段不会做什么</strong><span>不解析金额和票号，不判定有效发票，不去重，不归组，不创建费用。</span></aside>{#if error}<p class="feedback error" role="alert">{error}</p>{/if}<footer><button class="secondary" type="button" onclick={() => (showCreate = false)}>取消</button><button class="primary" type="submit" disabled={creating || !session.configured}>{creating ? '创建中…' : '创建任务'}</button></footer></form></div></div>{/if}
{#if deleteCandidate}<ConfirmDialog title="删除收集任务" message={`将删除“${deleteCandidate.name}”的本地邮件台账和收集材料，不会删除邮箱中的邮件。审核完成或已导入报销批次的任务为保护来源记录不能删除。`} confirmLabel="删除任务" tone="danger" busy={deletingTaskId === deleteCandidate.id} onConfirm={() => void deleteTask()} onCancel={() => (deleteCandidate = null)} />{/if}
