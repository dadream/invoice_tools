...
/// 构建城市链（识别中转点）- 旧版本，保留以防需要
fn build_city_chain(
    intercity_ids: &[usize],
    all_invoices: &[ParsedInvoice],
    home_cities: &[String],
) -> Vec<String> {
    build_city_chain_with_ambiguities(intercity_ids, all_invoices, home_cities).0
}
...
fn detect_no_return_ticket(
    ...
    for trip in trips {
        if let TripKind::BusinessTrip { .. } = trip.kind {
            // 检查最后一张交通票是否回到常驻城市
            let intercity_in_trip: Vec<usize> = trip.invoice_ids.iter()
                .filter(|&&id| {
                    matches!(invoices[id].ticket_type, TicketType::Rail | TicketType::Flight)
                })
                .copied()
                .collect();
...
fn detect_time_overlap(
    ...
    for (idx, inv) in invoices.iter().enumerate() {
        if matches!(inv.ticket_type, TicketType::Rail | TicketType::Flight) {
            if let Some(dt) = inv.departure_time {
                same_day_transports.entry(dt.date()).or_default().push((idx, inv));
            }
        }
    }
    ...
    for (date, transports) in same_day_transports {
        if transports.len() >= 2 {
            // 检查是否有不同目的地
            let destinations: Vec<String> = transports.iter()
...
fn detect_transfer_stopover(
    ...
    for trip in trips {
        if let TripKind::BusinessTrip { .. } = trip.kind {
            // 检查行程中的连续交通票
            let intercity_ids: Vec<usize> = trip.invoice_ids.iter()
                .filter(|&&id| matches!(invoices[id].ticket_type, TicketType::Rail | TicketType::Flight))
                .copied()
                .collect();
            ...
            for i in 0..intercity_ids.len().saturating_sub(1) {
                ...
                if let (Some(curr_time), Some(next_time)) = (curr_inv.departure_time, next_inv.departure_time) {
                    let interval_hours = (next_time - curr_time).num_hours();

                    // 4-12h 灰色区间
                    if interval_hours >= TRANSFER_THRESHOLD_HOURS && interval_hours < STOPOVER_THRESHOLD_HOURS {
                        if let Some(dest) = extract_destination(curr_inv) {
                            ambiguities.push(Ambiguity {
                                kind: AmbiguityKind::TransferStopover,
                                description: format!("在 {} 停留 {} 小时，中转还是行程点？", dest, interval_hours),
                                involved_invoice_ids: vec![intercity_ids[i], intercity_ids[i + 1]],
                                candidates: vec![
                                    "中转点，不计入行程城市".to_string(),
                                    "行程点，计入行程城市".to_string(),
...
fn detect_weekend_between_trips(
    ...
    // 找出所有从常驻城市出发的交通票
    let departures_from_home: Vec<(usize, &ParsedInvoice)> = invoices.iter()
        .enumerate()
        .filter(|(_, inv)| {
            matches!(inv.ticket_type, TicketType::Rail | TicketType::Flight)
                && inv.city.as_ref().map(|c| is_home_city(c, home_cities)).unwrap_or(false)
        })
        .collect();
            ...
            if gap_days >= 2 && gap_days <= 4 {
                ...
                if weekday1.number_from_monday() >= 5 && weekday2.number_from_monday() == 1 {
                    ambiguities.push(Ambiguity {
                        ...
                        description: format!("周末夹在两次出发之间（{} 到 {}）", time1.date(), time2.date()),
