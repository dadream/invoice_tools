...
> 状态：已确认的分阶段设计；功能 MVP 保持无账号、无应用层加密
> 日期：2026-09-03
...
1. **功能 MVP**：完成独立邮件收集、批次解析、校验、归组、审核、Excel/打印 PDF 和 Concur 草稿交付准备；使用本地单用户模型，不注册、不登录，不持久化邮箱授权码。显式备份为未加密可移植包并提示风险；Concur 真实写入另行验证。
