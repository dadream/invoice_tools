...
> **修正说明（2026-08-06）**：原报告（基于 commit 5f324ce / e31a67f）中 L1 坐标文本框
> 路径（`pdf_text::parse_vat_invoice_from_boxes`）从未接入 `verify-all` 评分，
> pdf-vat 通过率严重低估。本次修正：
> - commit 41ce63b：将 L1 坐标路径接入 verify-all（pdf-vat 降级 flat-text 兜底）
> - commit 21f3875：修复多行金额配对（`locate_vat_fields` 支持大写行+小写行布局）
> 
> 原报告结论"与 Task 7.5 持平（15.6%）= 管道稳定"有误；实际是 L1 代码从未参与评分，
> 不是稳定，而是未测量。本报告数字已更新为真实基线。
...
| 失败原因 | 4 个样本字段定位失败（无内嵌发票 XML 或文本框坐标解析异常） |

**结论:** ⚠️ **部分可用**。OFD 容器内含完整发票 XML 或布局文本可被坐标路径解析时通过；纯布局文件需 L2 (OCR)。

**通过样本:**
- 11-meituan-34ee412d.ofd ✅
- 40-meituan-12f8065e.ofd ✅
- 28-unknown-36c9093e.ofd ✅（本次修复后新增）
- 33-unknown-1f1e61a4.ofd ✅（本次修复后新增）
...
| 通过数 | 12 |
| **通过率** | **29.3%** |
| 解析级别 | L1 (PDF text boxes + coordinates) + L0 fallback |
| 部分匹配数 | 11 (核心字段正确，可选字段缺失) |

**结论:** ⚠️ **核心字段可用，可选字段需进一步改进**

**字段提取准确率（基于已标注的 11 个 PDF-VAT 样本）:**
- ✅ invoice_number: ~90%
- ✅ issue_date: ~90%
- ✅ total_amount: ~95%（多行金额布局修复后）
- ❌ tax_amount: ~10%（坐标路径尚未覆盖此字段的典型布局）
- ❌ buyer_name: ~10%
- ❌ seller_name: ~10%
...
原始基线（commit e31a67f，L1 路径未接入评分）与本次真实基线对比：

| 格式 | 原始基线（误报） | Task 1 基线（L1 接入） | Task 2 基线（金额修复） | 变化说明 |
|---|---|---|---|---|
| xml-vat | 100.0% (7/7) | 100.0% (7/7) | 100.0% (7/7) | 持平 ✅ |
| ofd | 25.0% (2/8) | 25.0% (2/8) | **50.0% (4/8)** | 多行金额修复 +25% ⬆ |
| pdf-vat | 2.4% (1/41) | **29.3% (12/41)** | 29.3% (12/41) | L1 接入 +26.9% ⬆ |
| pdf-flight | 0.0% (0/2) | 0.0% (0/2) | 0.0% (0/2) | 未标注 |
| image | 0.0% (0/6) | 0.0% (0/6) | 0.0% (0/6) | 需 OCR sidecar |
| **总计** | **15.6% (10/64)** | **32.8% (21/64)** | **35.9% (23/64)** | **+20.3% ⬆** |

**核心结论：** pdf-vat 从 2.4% 提升至 29.3%（+26.9%），是 L1 坐标路径接入的直接效果，
而非管道退步或持平。原报告"持平"结论源于评分路径错误，已废弃。
